 ...
 */
const OverlayCanvas = forwardRef(
  ({ width, height, visible = true, isDrawingEnabled = false }, ref) => {
    ...
    const handleMouseDown = (e) => {
      if (!visible || !isDrawingEnabled) return; // 添加 isDrawingEnabled 检查
    ...
    const handleCanvasClick = (e) => {
      if (!visible || !isDrawingEnabled || currentTool !== "text") return; // 添加 isDrawingEnabled 检查
      const rect = canvasRef.current.getBoundingClientRect();
      const point = {
        x: e.clientX - rect.left,
        y: e.clientY - rect.top,
      ...
      <>
        <canvas
          ...
          style={{
            ...
            top: 0,
            left: 0,
            pointerEvents: visible && isDrawingEnabled ? "auto" : "none",
            opacity: visible ? 1 : 0,
